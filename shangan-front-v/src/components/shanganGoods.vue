<template>
  <div class="good-item">
    <div style="">
      <div class="good-img">
        <a @click="openProduct(msg.goodsId)">
          <img v-lazy="msg.goodsCoverImg" :alt="msg.goodsName" :key="msg.goodsCoverImg">
        </a>
      </div>
      <h6 class="good-title" v-html="msg.goodsName">{{msg.goodsName}}</h6>
      <h3 class="sub-title ellipsis">{{msg.goodsIntro}}</h3>
      <div class="good-price pr">
        <div class="ds pa">
          ...
          <y-button text="加入购物车"
                    style="margin: 0 5px"
                    @btnClick="addCart(msg.goodsId,msg.sellingPrice,msg.goodsName,msg.goodsCoverImg)"
                    classStyle="main-btn"
          ></y-button>
        </div>
        <p><span style="font-size:14px">￥</span>{{Number(msg.sellingPrice).toFixed(2)}}</p>
      </div>
    </div>
  </div>
</template>
